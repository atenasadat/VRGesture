...
#
#     self.df = pd.concat([self.df,pd.read_csv((Path(self.path2data)/'cmu_intervals_df_transforms.csv').as_posix())]) ## file with evil twins
...
#                               'pin_memory':False}
#     print("dataloader args", self.dataLoader_kwargs)
...
#       interval_list.append(MiniData(self.getPath2file(interval_id),
#                                     style=self.getStyle(interval_id),
#                                     **self.minidataKwargs))
...
#
#       print("-------- train size: {} test size: {} val size:{}" .format(len(df_train), len(df_test),len(df_dev)))
#     else:
...
#     print("Missing intervals loaded")
...
#     print("*********train interval size: {} test size: {} val size: {}".format(len(train_intervals), len(test_intervals), len(dev_intervals)))
#
#     self.train_intervals_all = train_intervals
#     self.dev_intervals_all = dev_intervals
#     self.test_intervals_all = test_intervals
#     print("self.laod data is", self.load_data)
...
#         # print("{} file opend with modality {}".format(path2h5 , modality))
#       except:
#         # data, h5 = self.load(self.path2h5, 'text')
#         print("error : {} file with madality {}".format(path2h5,modality))
#         continue
#         # sys.exit(1)
#
...
#         indices_arr = np.array(indices).astype(int)
...
#         item.update({'text/token_duration':length_word.astype(int)})
#
#       ## close h5 file
#       #h5.close()
#
...
class Data(Modality):
  ...
  def tdt_split(self):
    ...
    ## update_train_intervals
    train_intervals, dev_intervals, test_intervals, train_intervals_dict = self.update_intervals(train_intervals,
                                                                                                 dev_intervals,
                                                                                                 test_intervals)
  ...
  def update_dataloaders(self, time, window_hop):
    ...
    self.train = DataLoader(ConcatDatasetIndex(self.datasets['train'].datasets), sampler=self.train_sampler,
...
class MiniData(Dataset, HDF5):
  def __init__(self, path2h5, modalities, fs_new, time, modality_classes, window_hop, style=0, repeat_text=1,
               text_in_modalities=False, filler=0, audio_average=0, **kwargs):
    super(MiniData, self).__init__()
    self.path2h5 = path2h5
    self.modalities = modalities
    self.fs_new = fs_new
...
class DataSample(Data):
  ...
  def get_minidata_list(self, intervals):
    new_intervals = list(filter(None,
                                [interval_id if os.path.exists(self.getPath2file(interval_id)) else None for interval_id
                                 in intervals]))
    return [MiniData(self.getPath2file(interval_id), style=self.getStyle(interval_id), **self.minidataKwargs)
...
class AlternateClassSampler(Sampler):
  ...
  def __iter__(self):
    return iter(torch.stack(
      [torch.randint(start, end, size=(self.num_samples_per_class,)) for start, end in zip(self.starts, self.ends)],
      dim=1).view(-1).tolist())
...
class BalanceClassSampler(Sampler):
  ...
  def __iter__(self):
    return iter(torch.stack(
      [class_idx[torch.randint(0, len(class_idx), size=(self.num_samples_per_class,))] for class_idx in self.classes],
      dim=1).view(-1).tolist())

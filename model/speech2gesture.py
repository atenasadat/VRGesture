...
class Speech2Gesture_G(nn.Module):
  ...
  '''
  def __init__(self, time_steps=64, in_channels=256, out_feats=96, p=0):
    ...
    self.decoder = nn.Sequential(*nn.ModuleList([ConvNormRelu(in_channels, in_channels,
                                                              ...
                                                              p=p)
                                  for i in range(4)]))
    self.logits = nn.Conv1d(in_channels, 96, kernel_size=1, stride=1)
...
class Speech2Gesture_D(nn.Module):
  ...
  def __init__(self, in_channels=104, out_channels=64,  n_downsampling=2, p=0, groups=1, **kwargs):
    ...
    self.conv1 = nn.Sequential(torch.nn.Conv1d(104*groups, out_channels*groups, 4, 2, padding=1, groups=groups),
